...
- `.github/workflows/release.yml` runs every offline gate **before** spending a
  signed build, then builds, audits, and publishes to GitHub Releases. Secrets:
  `KEYSTORE_BASE64`, `KEYSTORE_PASSWORD`, `KEY_ALIAS`, `KEY_PASSWORD`.
- `.github/workflows/ci.yml` is **manual dispatch only**, on purpose. Do not
  re-enable per-push runs, and do not enable a workflow the owner disabled.
- A release stays **prerelease** until a physical run has confirmed the whole
  chain. Publishing a full release is the owner's call.
- The release must contain both APKs, `SHA256SUMS.txt` and
  `build-provenance.txt`, and the `assets/df_reroot.apk` bundled inside the
  installer must be **byte-identical** to the published `df_reroot.apk`
  (`tools/ci_build_audit.sh` proves this).
